...
const stickyElement = (x,y, hoveredEI) => {
    ...
    if(hoveredEI.classList.contains("sticky")){
        ...
        if(hoveredEI.offsetTop <= hoveredEIPosition[0] - 100 ||
...
const mouseCircleTransform = (hoveredEI) => {
    if(hoveredEI.classList.contains("pointer-enter")){
        hoveredEI.onmousemove = () => {
            ...
            animation: none;
            border-radius: ${getComputedStyle(hoveredEI).borderBottomLeftRadius};
...
const progressBarFn = () => {
    ...
    const atLastSection = scrolledPortion + pageViewportHeight >= lastSectionPosition;
...
progressBar.onclick = (e) => {
    ...
    const nextSectionPosition = sectionPositions.find(position => position > scrolledPortion);
...
const aboutMeTextContent = "I am a backend developer & I do not talk much, just contact me :)";
...
const hideProjects = (project, i) => {
    ...
    setTimeout(() =>{project.style.opacity = "0"; 
    }, i*100);
...
document.querySelectorAll(".service-btn").forEach((service) => {
    ...
    service.addEventListener("mouseover", () => {
        ...
        const rightPosition = `calc(100% - ${getComputedStyle(service.firstElementChild).width})`;
